...
public class AbaseGestureAllActivity extends AbaseAllActivity implements OnGestureListener
{
	...
	protected Dir getGestureDirection(MotionEvent e1, MotionEvent e2)
	{
		if (e2.getX() - e1.getX() > MIN_GESTURE_DISTANCE) return Dir.RIGHT;
		if (Math.abs(e2.getX() - e1.getX()) > MIN_GESTURE_DISTANCE) return Dir.LEFT;
		if (Math.abs(e2.getY() - e1.getY()) > MIN_GESTURE_DISTANCE) return Dir.UP;
		if (e2.getY() - e1.getY() > MIN_GESTURE_DISTANCE) return Dir.DOWN;
