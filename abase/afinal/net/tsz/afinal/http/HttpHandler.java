...
public class HttpHandler<T> extends AsyncTask<Object, Object, Object> implements EntityCallBack
{
	 ...
	 * @param totalLength
	 */
	public HttpHandler(AbstractHttpClient client, HttpContext context, AjaxCallBack<T> callback, String charset, long totalLength)
	{
		this.client = client;
	...
	private void makeRequestWithRetries(HttpUriRequest request) throws IOException
	{
		if (isResume && targetUrl != null)
		{
			...
			if (fileLen > 0)
			{
				...
				else if (totalLenth != -1) request.setHeader("RANGE", "bytes=" + fileLen + "-" + (totalLenth-1));
				else
				{
					if (downloadFile.isFile() && downloadFile.exists())
					{
	...
	protected void onProgressUpdate(Object... values)
	{
		...
		case UPDATE_START:
			...
			break;
		case UPDATE_LOADING:
			if (callback != null) callback.onLoading(Long.valueOf(String.valueOf(values[1])), Long.valueOf(String.valueOf(values[2])));
			break;
		case UPDATE_FAILURE:
			if (callback != null) callback.onFailure((Throwable) values[1], (Integer) values[2], (String) values[3]);
			break;
		case UPDATE_SUCCESS:
